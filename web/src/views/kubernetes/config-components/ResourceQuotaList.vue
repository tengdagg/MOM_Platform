...
const filteredResourceQuotas = computed(() => {
  ...
  if (selectedNamespaces.value.length > 0 && !selectedNamespaces.value.includes('')) {
    result = result.filter(rq => selectedNamespaces.value.includes(rq.namespace))
  }
...
const loadNamespaces = async () => {
  ...
  try {
    ...
    kubernetesStore.setAllNamespaces(data || [])
  } catch (error) {
    namespaces.value = []
    kubernetesStore.setAllNamespaces([])
  }
}
...
.action-btn {
  color: var(--el-color-primary); /* Use Element Plus primary color variable */
  padding: 4px;
}

.action-btn:hover {
  color: var(--el-color-primary-light-3); /* Lighter primary on hover */
}

.action-btn.danger {
  color: var(--el-color-danger); /* Use Element Plus danger color variable */
}

.action-btn.danger:hover {
  color: var(--el-color-danger-light-3); /* Lighter danger on hover */
}
