  ...
  <div class="list-container">
    <div class="search-bar">
      <div class="search-bar-left">
        <el-input
          v-model="searchName"
          placeholder="搜索 MutatingWebhookConfiguration 名称..."
          clearable
          class="search-input"
          @input="handleSearch"
        >
...
const yamlDialogTitle = computed(() => {
  return isCreateMode.value ? '创建 MutatingWebhook' : `编辑 MutatingWebhook - ${selectedItem.value?.name}`
})
...
const loadData = async () => {
  ...
  try {
    ...
    const response = await axios.get('/api/v1/plugins/kubernetes/resources/mutatingwebhookconfigurations', {
...
const handleEdit = async (row: WebhookConfigInfo) => {
  ...
  try {
    const token = localStorage.getItem('token')
    const response = await axios.get(`/api/v1/plugins/kubernetes/resources/mutatingwebhookconfigurations/${row.name}/yaml`, {
      params: { clusterId: props.clusterId },
      headers: { Authorization: `Bearer ${token}` }
    })
...
const handleSaveYAML = async () => {
  ...
  try {
    ...
    if (isCreateMode.value) {
      await axios.post(`/api/v1/plugins/kubernetes/resources/mutatingwebhookconfigurations/yaml`, {
      ...
      await axios.put(`/api/v1/plugins/kubernetes/resources/mutatingwebhookconfigurations/${name}/yaml`, {
        clusterId: props.clusterId,
        yaml: yamlContent.value
      }, {
        headers: { Authorization: `Bearer ${token}` }
...
const handleDelete = (row: WebhookConfigInfo) => {
  ElMessageBox.confirm(
    `确定要删除 MutatingWebhookConfiguration ${row.name} 吗？`,
    '删除确认',
    {
    ...
    try {
      const token = localStorage.getItem('token')
      await axios.delete(`/api/v1/plugins/kubernetes/resources/mutatingwebhookconfigurations/${row.name}`, {
        params: { clusterId: props.clusterId },
        headers: { Authorization: `Bearer ${token}` }
      })
...
defineExpose({ loadMutatingWebhooks: loadData })
</script>
