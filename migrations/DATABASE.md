...
```sql
-- 创建应用用户（不使用root）
CREATE USER 'mom'@'localhost' IDENTIFIED BY 'strong_password';
GRANT ALL PRIVILEGES ON mom.* TO 'mom'@'localhost';
FLUSH PRIVILEGES;

-- 用于生产环境的只读副本用户
CREATE USER 'mom_readonly'@'localhost' IDENTIFIED BY 'readonly_password';
GRANT SELECT ON mom.* TO 'mom_readonly'@'localhost';
FLUSH PRIVILEGES;
```
...
```bash
# 备份所有数据
mysqldump -u root -p mom > mom_full.sql

# 仅备份表结构
mysqldump -u root -p -d mom > mom_schema.sql

# 压缩备份
mysqldump -u root -p mom | gzip > mom_$(date +%Y%m%d).sql.gz

# 恢复备份
mysql -u root -p mom < mom_full.sql
```
...
BACKUP_FILE="$BACKUP_DIR/mom_$DATE.sql.gz"

mysqldump -u mom -p'password' mom | gzip > "$BACKUP_FILE"

# 保留最近30天的备份
find "$BACKUP_DIR" -name "mom_*.sql.gz" -mtime +30 -delete
```
