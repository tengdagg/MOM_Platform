...
优先选择 `task.execute` 的场景：

- 用户明确要求按主机分组批量执行
- 用户说"在生产环境所有主机"、"在 Web 组全部机器"、"统一批量执行一个命令"
- 目标是一次性 Ad-hoc 批处理，而不是登录单台主机持续排障

以下场景不要优先选它，应改用 `host.exec_command`：

- 用户只给了一台主机 IP 或单个主机 ID
- 用户是在单机排障、修改、验证，且后续可能连续执行多条命令
- 用户需要依赖当前目录、环境变量或 shell 状态的交互式上下文
- 用户没有明确给出 `group_name`，也没有表达"整组/全部主机"的意图
